...
class CustomPortfolioEnv(gym.Env):
    ...
    def step(self, action):
        ...
        daily_returns = daily_portfolio_values[1:] / daily_portfolio_values[:-1] - 1 if len(
            daily_portfolio_values) > 1 else np.array([0])
        ...
        drawdown = (cummax - daily_portfolio_values) / cummax if cummax[0] > 0 else np.zeros_like(
...
class CustomPortfolioEnvBacktest(gym.Env):
    def __init__(self, price_dir="metrics_used_6assets/price", metrics_dir="organized_6assets/observation/metrics"):
        ...
        self.backtest_months = pd.date_range(start=METRICS_BACKTEST_START, end=METRICS_BACKTEST_END,
...
class CustomPortfolioEnvNLPBacktest(gym.Env):
    def __init__(self, price_dir="metrics_used_6assets/price", nlp_metrics_dir="observation/metrics"):
        ...
        # Define backtest period
        self.backtest_months = pd.date_range(start=NLP_BACKTEST_START, end=NLP_BACKTEST_END, freq="ME").strftime(
            "%Y-%m").tolist()
        for month in self.backtest_months:
            if month not in self.observations:
