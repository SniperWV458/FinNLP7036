...
class StockTwitsDataCleaner:
    ...
    def generate_summary_report(self):
        ...
        for asset_name, stats in self.statistics.items():
            ...
            report.append(f"    Characters: mean {stats['length_before']['char_mean']:.1f}, median {stats['length_before']['char_median']:.1f}")
            report.append(f"    Words: mean {stats['length_before']['word_mean']:.1f}, median {stats['length_before']['word_median']:.1f}")
            
            report.append(f"  Text length statistics - After cleaning:")
            report.append(f"    Characters: mean {stats['length_after']['char_mean']:.1f}, median {stats['length_after']['char_median']:.1f}")
            report.append(f"    Words: mean {stats['length_after']['word_mean']:.1f}, median {stats['length_after']['word_median']:.1f}")
            
            report.append(f"  Truncated entries: {stats['truncated_count']:,}")
            report.append(f"  Character ratio - Before: English {stats['char_ratio_before']['english']:.2%}, Digits {stats['char_ratio_before']['digit']:.2%}")
            report.append(f"  Character ratio - After: English {stats['char_ratio_after']['english']:.2%}, Digits {stats['char_ratio_after']['digit']:.2%}")
            ...
            report.append(f"  Direction keywords - Before: Positive {stats['direction_count_before']['positive']:,}, Negative {stats['direction_count_before']['negative']:,}")
            report.append(f"  Direction keywords - After: Positive {stats['direction_count_after']['positive']:,}, Negative {stats['direction_count_after']['negative']:,}")
